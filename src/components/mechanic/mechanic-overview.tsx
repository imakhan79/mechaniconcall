    ...
    <div className="mx-auto max-w-4xl px-4 py-8">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          disabled={toggling}
          className={`flex items-center gap-2 rounded-full px-4 py-2 text-sm font-bold ${isOnline ? "bg-green-100 text-green-700" : "bg-neutral-200 text-neutral-600"}`}
        >
          <span className={`h-2.5 w-2.5 rounded-full ${isOnline ? "bg-green-600" : "bg-neutral-400"}`} />
          ...
          <Card className="mt-6 border-2 border-orange-300">
            <CardContent className="flex items-center justify-between py-4">
              <div className="flex items-center gap-3">
                {activeJob.is_emergency ? <AlertTriangle className="h-7 w-7 text-red-600" /> : <MapPin className="h-7 w-7 text-orange-600" />}
                <div>
