...
export default async function MechanicEarningsPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <h1 className="font-heading text-xl font-bold text-neutral-900">Earnings</h1>
      ...
      <div>
        <h2 className="mb-2 text-sm font-semibold text-neutral-900">Payout History</h2>
        <Card>
          <CardContent className="divide-y divide-neutral-100 p-0">
            {(payouts ?? []).map((p) => (
              <div key={p.id} className="flex items-center justify-between p-4">
                <div>
                  <p className="text-sm font-medium text-neutral-900">Rs {Number(p.amount).toFixed(0)}</p>
                  <p className="text-xs text-neutral-400">{format(new Date(p.requested_at), "d MMM yyyy")}</p>
                </div>
            ...
            {(!payouts || payouts.length === 0) && (
              <p className="p-5 text-center text-sm text-neutral-400">No payout requests yet.</p>
            )}
          </CardContent>
        </Card>
      </div>
      ...
      <div>
        <h2 className="mb-2 text-sm font-semibold text-neutral-900">Completed Jobs</h2>
        <Card>
          <CardContent className="divide-y divide-neutral-100 p-0">
            {(earnings ?? []).map((e) => (
              <div key={e.id} className="flex items-center justify-between p-4">
                <div>
                  <p className="text-sm font-medium text-neutral-900">Job #{e.request_id}</p>
                  <p className="text-xs text-neutral-400">{format(new Date(e.created_at), "d MMM yyyy")}</p>
                </div>
                <p className="text-sm font-semibold text-neutral-900">Rs {Number(e.net_amount).toFixed(0)}</p>
              </div>
            ))}
            {(!earnings || earnings.length === 0) && (
              <p className="p-5 text-center text-sm text-neutral-400">No completed jobs yet.</p>
            )}
          </CardContent>
        </Card>
      </div>
  ...
  return (
    <Card>
      <CardContent className="p-4 text-center">
        <p className={`text-lg font-bold ${highlight ? "text-sky-700" : "text-neutral-900"}`}>Rs {value.toFixed(0)}</p>
