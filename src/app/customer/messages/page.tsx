        ...
        <CardContent className="divide-y divide-neutral-100 p-0">
          {(requests ?? []).map((r) => {
            const mechanic = r.mechanic as unknown as
              ...
              | undefined;
            return (
              <Link key={r.id} href={`/track/${r.id}`} className="flex items-center gap-3 p-4 hover:bg-neutral-50">
          ...
          {(!requests || requests.length === 0) && (
            <p className="p-5 text-center text-sm text-neutral-400">No conversations yet.</p>
          )}
        </CardContent>
      </Card>
    </div>
