    ...
    <div className="mx-auto max-w-4xl px-4 py-8">
      ...
      {active && (
        <Link href={`/track/${active.id}`}>
          <Card className={`mt-6 border-2 ${active.is_emergency ? "border-red-300" : "border-orange-300"}`}>
            <CardContent className="flex items-center justify-between py-4">
              <div className="flex items-center gap-3">
                {active.is_emergency ? <AlertTriangle className="h-8 w-8 text-red-600" /> : <MapPin className="h-8 w-8 text-orange-600" />}
                <div>
      ...
      <div className="mt-8">
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-2">
          ...
          {(vehicles ?? []).length === 0 && (
            <Link href="/customer/vehicles" className="flex items-center justify-center rounded-xl border border-dashed border-neutral-300 p-6 text-sm text-neutral-500 hover:border-orange-300">
            ...
            <Card key={h.id}>
              <CardContent className="flex items-center justify-between py-3">
                ...
                <div className="flex items-center gap-1 text-sm font-semibold text-neutral-700">
                  <Star className="h-3.5 w-3.5 fill-amber-400 text-amber-400" /> {h.final_price ? `PKR ${h.final_price}` : "—"}
                </div>
              </CardContent>
            </Card>
          ))}
